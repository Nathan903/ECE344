...
import pexpect, sys, os, shutil, signal, time
...
class TestUnit(object):
    ...
    def runprogram(self, cmd, args = ""):
        ...
        try:                
            ...
            while 1:
                try:
                    index = self.kernel.expect([self.prompt, pexpect.EOF], timeout=2)
                    if self.kernel.before:
                        print(self.kernel.before)
...
for t in testbin:
    log_string=o = test.runprogram("/testbin/"+t)
    exit()
    print(t)
    ...
    # Convert matches to set to get unique translations
    unique_translations = sorted(tuple(set(matches)))

    # Print unique translations
    for translation in unique_translations:
        print("\t",translation)

    # cd ../build && python3 test.py && cd ../os161
