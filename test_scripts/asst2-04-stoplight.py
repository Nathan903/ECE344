...
def main():
    ...
    max_occupied_regions = stoplight(test, nr_cars, all_cars)
    ...
    if result == nr_cars:
        if max_occupied_regions <= 1:
            print ('WARNING: maximum regions that were occupied is ' + str(max_occupied_regions) + '. This likely indicates that you are not allowing sufficient concurrency in the intersection (not allowing two non-conflicting cars in the intersection at the same time). Please double-check if you are holding a big lock on the entire intersection. Since the auto tester is non-deterministic, it could also be caused by a particular scheduling while your code is correct. If you think your solution is correct, you should ignore this warning and submit your solution. We will manually look into your code.')
            mark -= 20
