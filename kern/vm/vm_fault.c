...
int vm_fault(int faulttype, vaddr_t faultaddress) {
  ...
  if ((faultaddress >= vbase1 && faultaddress < vtop1)|| (faultaddress >= vbase2 && faultaddress < vtop2) || ( faultaddress >= stackbase && faultaddress < stacktop) || (faultaddress >= as->heap_start && faultaddress < as->heap_end) ) {
    ...
    if(i<PT_LENGTH){// found
      ...
      if(paddr>0 && paddr<SWAPSIZE){
        //rp("UNEVICT START");kprintf("%d %x %x\n", curthread->pid, faultaddress, paddr);
        paddr = unevict(paddr, &as->pagetable[i].pa);
        //rp("UNEVICT END\n");

        /*////////////////////////////////////////////////////
        kprintf("UNEVICTPT%p\n", as);
	    unsigned int ii; for(ii=0; ii<PT_LENGTH;ii++){
	    if (as->pagetable[ii].va!=0 || 0!=as->pagetable[ii].pa)
            kprintf("||%u|%x %x\n",ii,as->pagetable[ii].va,as->pagetable[ii].pa);
        }
        *////////////////////////////////////////////////////
        // rp("1\n");
      }
      ...
      if((faultaddress >= vbase2 && faultaddress < vtop2) ){
        ...
        loadseg(as->vc, faultaddress,as->phc);
